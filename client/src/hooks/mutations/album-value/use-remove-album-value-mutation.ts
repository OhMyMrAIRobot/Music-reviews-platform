 ...
 * related queries so the UI reflects the removal of the album value vote.
 *
 * @param {UseMutationParams} [options] - Optional lifecycle callbacks to forward to the underlying `useMutation` hook.
 * @returns The React Query mutation object for removing album value vote.
 */
export const useRemoveAlbumValueMutation = ({
	onSuccess,
	onError,
	onSettled,
}: UseMutationParams = {}) => {
