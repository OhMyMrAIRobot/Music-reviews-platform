 ...
 * author. On success the hook:
 *  - shows a success notification;
 *  - invalidates related queries so the client state stays up-to-date.
 *
 * @param {UseMutationParams} [options] - Optional lifecycle callbacks forwarded to the underlying `useMutation` hook.
 * @returns The React Query mutation object for creating an author.
 */
export const useAdminCreateAuthorMutation = ({
	onSuccess,
	onError,
	onSettled,
}: UseMutationParams = {}) => {
